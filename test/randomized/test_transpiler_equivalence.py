...
settings.register_profile(default_profile ,
...
settings.load_profile(os.getenv(u"HYPOTHESIS_PROFILE", default_profile))
...
def fully_supports_scheduling(backend):
    """Ensures that backend is not in the set of backends known not to have specified gate durations."""
...
mock_backends_with_durations = [b for b in mock_backends if fully_supports_scheduling(b)]
...
class QCircuitMachine(RuleBasedStateMachine):
    ...
    def transpiler_conf(draw):
        ...
        if scheduling_method is not None or backend_needs_durations:
            compatible_backends = st.sampled_from(mock_backends_with_durations)
